...
export async function embedPassages(passages: string[]) {
  ...
  const deadline = startedAt + INGESTION_LIMITS.targetDurationMs;
  for (let offset = 0; offset < passages.length; offset += INGESTION_LIMITS.embeddingBatchSize) {
    const remainingMs = deadline - Date.now();
    if (remainingMs <= 0) throw new EmbeddingProviderError("ingestion_timeout", true);
    const input = passages.slice(offset, offset + INGESTION_LIMITS.embeddingBatchSize).map(text => `passage: ${text}`);
    const response = await requestBatch(apiKey, input, deadline);
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      response = await fetch("https://openrouter.ai/api/v1/embeddings", {
        ...
        signal: AbortSignal.timeout(Math.min(INGESTION_LIMITS.providerTimeoutMs, remainingMs)),
      });
    } catch {
